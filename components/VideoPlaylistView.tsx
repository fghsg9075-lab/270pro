...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  // USAGE TRACKING & TRIAL LIMITER
  useEffect(() => {
      if (!activeVideo) return;
      
      // CHECK IF VIDEO IS ALREADY LOCKED (Used Trial Before)
      if (isTrial) {
          const lockedVideos = JSON.parse(localStorage.getItem('nst_locked_videos') || '[]');
          if (lockedVideos.includes(activeVideo.url)) {
              setActiveVideo(null);
              setAlertConfig({
                  isOpen: true,
                  message: "⛔ Trial Expired for this video.\n\nPlease purchase a subscription to continue watching."
              });
              return;
          }
      }
      ...
      const interval = setInterval(() => {
          ...
          setPlayedSeconds(currentDuration);

          if (isTrial && currentDuration >= 120) { // 2 Minutes
              // LOCK VIDEO PERMANENTLY (Local)
              const lockedVideos = JSON.parse(localStorage.getItem('nst_locked_videos') || '[]');
              if (!lockedVideos.includes(activeVideo.url)) {
                  lockedVideos.push(activeVideo.url);
                  localStorage.setItem('nst_locked_videos', JSON.stringify(lockedVideos));
              }
              ...
              setAlertConfig({
                  isOpen: true, 
                  message: "⏳ Trial Ended! You watched 2 minutes.\n\nPurchase a Subscription to watch full videos."
              });
      ...
      return () => {
          ...
          if (duration > 5) {
              ...
              // Update User
              // NOTE: user prop is immutable here, we need to clone.
              // BUT we are inside a closure. We should use a functional update if possible or just use current user prop.
              // We rely on parent update function.
              // However, this cleanup runs when activeVideo changes.
              
              // We need to fetch FRESH user data or trust that parent handles concurrency?
              // Let's just create a new history array.
              // WARNING: usageHistory might be large.
              
              // We will append to a local storage buffer for now to avoid frequent cloud writes?
              // No, let's update user directly.
  ...
  const handleVideoClick = (index: number) => {
      ...
      // 3. Check Trial Eligibility (If Ultra/Paid but User is Free)
      let trialMode = false;
      ...
      if (video.access === 'FREE') {
          ...
          if (isSubscribed && (user.subscriptionLevel === 'BASIC' || user.subscriptionLevel === 'ULTRA')) {
           ...
           if (isSubscribed && (user.subscriptionTier === 'YEARLY' || user.subscriptionTier === 'LIFETIME')) {
               ...
               // ALLOW TRIAL FOR ULTRA VIDEOS IF NOT SUBSCRIBED
               // Requirement: "free user 2 min tak dekh sakta hai tarayal"
               trialMode = true;
           }
      }
      ...
      if (trialMode) {
          setIsTrial(true);
          triggerVideoPlay(video);
          // Show Toast or Alert about Trial?
          // Maybe just start playing. The popup will come at 2 mins.
          // Let's show a small alert first? No, direct play is better UX.
          // But we should notify them "Trial Mode: 2 Mins".
          setAlertConfig({isOpen: true, message: "🎥 Trial Started: You can watch this Premium Video for 2 minutes."});
          return;
      }
